...
```{r eval=TRUE}
#data2comb <- rbind(trainTransformed, testTransformed)
#ntrees <- boosted_tree$bestTune[[1]] #n.trees
#nsplits <- boosted_tree$bestTune[[2]] #interaction.depth

#final_boosted_tree <- train(log.shares ~ . , data = trainTransformed,
#      method = "gbm", 
#      trControl = trainControl(method = "cv", number = 10), #method="repeatedcv", repeats=5
#      tuneGrid = data.frame(n.trees = ntrees, interaction.depth = nsplits, shrinkage = 0.1, n.minobsinnode = 10),
#      importance = TRUE,
#      verbose = FALSE)
...
print(paste0("the winnder is ", first, ", second is ", second ))
```
