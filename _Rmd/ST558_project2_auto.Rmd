...
#  kable(digits = 4, caption = "Table 5. Closeness to LDA topic 3 on Day of the Week")
...
#cv_fit2 <- train(log.shares ~ . + I(self_reference_avg_sharess^2),
#                 data=trainTransformed,
#                 method = "lm",
#                 trControl = trainControl(method = "cv", number = 10))
#summary(cv_fit2)

cv_fit3 <- train(log.shares ~ . + I(self_reference_avg_sharess^2) + 
                 kw_avg_avg:num_imgs + self_reference_avg_sharess:num_imgs + average_token_length:global_subjectivity, 
                 data=trainTransformed,
                 method = "lm",
                 trControl = trainControl(method = "cv", number = 10))
summary(cv_fit3)

result_tab <- data.frame(t(cv_fit1$results), t(cv_fit3$results))
colnames(result_tab) <- c("Model 1", "Model 3")
...
result2 <- rbind(cv_rmse1, cv_rmse3)
row.names(result2) <- c("Model 1", "Model 3")
