...
The channel for this analysis is `r channel`.
...
channels <- paste0("data_channel_is_", channel)
news <- all_news %>% filter(channels == 1) %>% select(
...
confus2 <- confusionMatrix(boosted_tree_predict, test2$class_shares)
confus2$overall[2]

#accuracy_test <- rbind(confus1$overall[1], confus2$overall[1])
#rownames(accuracy_test) <- c("Random Forest", "Boosted Tree")
#kable(accuracy_test, digits = 4, caption = "Accuracy of the candidate models on test set") 
