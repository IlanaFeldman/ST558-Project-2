...
We then used 5 fold cross validation to search for the tuning parameter value ranging from 1 to 5 that produces the optimal random forest model. The optimal model chosen by cross validation produced the smallest RMSE value when mtry = `r random_forest$bestTune[[1]]` and the lowest RMSE = `r min(random_forest$results[, 2])`.  
...
We then used 10 fold cross validation to search all combinations of the tuning parameters values using the `expand.grid` function to choose an optimal model with the desired tuning parameters values. The optimal boosted tree model chosen by cross validation produced the smallest RMSE value (`r min(boosted_tree$results[, 5])`) when n.trees = `r boosted_tree$bestTune[[1]]`, interaction.depth = `r boosted_tree$bestTune[[2]]`, shrinkage = `r boosted_tree$bestTune[[3]]` and n.minobsinnode = `r boosted_tree$bestTune[[4]]`. 

## Model Comparisons

The best model fit to predict the number of shares for the `r params$channel` channel can be determined by looking at the Root Mean Square Error, the Mean Absolute Error, or the R-squared value. The approach I've taken below picks whichever model is considered superior by the majority of these three metrics, and if each of the metrics picks a different model, then the one with the lowest RMSE will be chosen.
...
The best model fit to predict the number of shares is the `r final_model` model for the `r params$channel` articles.

both training and the test sets have been standardized with same mean and same standard deviation. 
